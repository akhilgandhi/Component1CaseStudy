...
public class StockPriceResource {
    ...
    public StockPriceDTO getStockByCompanyBetweenDates(@Parameter(description = "Company Code", required = true)
        ...
        try {
            ...
            stockPriceDTO.setStockPrices(stockPrices.stream()
                    .sorted(Comparator.comparing(StockPrice::getStockId, Comparator.reverseOrder()))
                    .collect(Collectors.toList()));
            stockPriceDTO.setMinStockPrice(stockPrices.stream()
                    .min(Comparator.comparing(StockPrice::getStockPrice)).get().getStockPrice());
            stockPriceDTO.setMaxStockPrice(stockPrices.stream()
                    .max(Comparator.comparing(StockPrice::getStockPrice)).get().getStockPrice());
            stockPriceDTO.setAvgStockPrice(stockPrices.stream().map(StockPrice::getStockPrice)
                    .reduce(BigDecimal.ZERO, BigDecimal::add).divide(BigDecimal.valueOf(stockPrices.stream().count())));
    ...
    public BigDecimal getLatestStockByCompanyCode(@PathVariable("companycode") Long companyCode) throws StockPriceNotFoundException {
        try {
